...
import {
    role as adminUsersRole,
    deleteMethod as adminUsersDelete,
} from '@/routes/admin/users';
import {
    resolve as adminErrorReportsResolve,
    deleteMethod as adminErrorReportsDelete,
} from '@/routes/admin/error_reports';
...
const statCards = (stats: AdminStats) => [
    ...
    {
        label: 'Con Moodle conectado',
        value: stats.usersWithMoodleConnected,
        icon: Wifi,
    },
    {
        label: 'Sin Moodle conectado',
        value: stats.usersWithoutMoodleConnected,
        icon: WifiOff,
    },
    {
        label: 'Con 2FA activo',
        value: stats.usersWithTwoFactorEnabled,
        icon: Shield,
    },
    {
        label: 'Notificaciones en background',
        value: stats.usersWithBackgroundNotifications,
        icon: Bell,
    },
    {
        label: 'Nuevos (7 días)',
        ...
        <>
            ...
            <main className="p-admin">
                ...
                <header className="p-admin__page-header">
                    <div
                        className="p-admin__page-header-icon"
                        aria-hidden="true"
                    >
                        <ShieldCheck size={16} />
                    </div>
                    <h1 className="p-admin__page-header-title">
                ...
                <div className="p-admin__body">
                    ...
                    <section aria-labelledby="stats-title">
                        <div className="p-admin__section-label">
                            <span
                                id="stats-title"
                                className="p-admin__section-label-text"
                            >
                                Estadísticas
                            </span>
                            <span
                                className="p-admin__section-label-line"
                                aria-hidden="true"
                            />
                        </div>
                                ...
                                ({ label, value, icon: Icon }) => (
                                    <div key={label} className="p-admin__stat">
                                        <div className="p-admin__stat-top">
                                            <span className="p-admin__stat-label">
                                                {label}
                                            </span>
                                            <Icon
                                                className="p-admin__stat-icon"
                                                aria-hidden="true"
                                            />
                                        </div>
                                        <span className="p-admin__stat-value">
                                            {value}
                                        </span>
                                    </div>
                                ),
                                ...
                                <table className="p-admin__table">
                                    ...
                                    <tbody>
                                        {users.data.map((user) => {
                                            ...
                                            return (
                                                <tr key={user.id}>
                                                    <td className="p-admin__table-cell--name">
                                                        {user.name}
                                                    </td>
                                                    ...
                                                    <td>
                                                        {user.moodle_connected ? (
                                                            <CheckCircle2
                                                                className="p-admin__status-icon p-admin__status-icon--ok"
                                                                aria-label="Conectado"
                                                            />
                                                        ) : (
                                                            <XCircle
                                                                className="p-admin__status-icon p-admin__status-icon--off"
                                                                aria-label="No conectado"
                                                            />
                                                        )}
                                                    </td>
                                                    <td>
                                ...
                                <table className="p-admin__table">
                                    ...
                                    <tbody>
                                        {errorReports.data.map((report) => (
                                            <tr key={report.id}>
                                                <td className="p-admin__table-cell--mono">
                                                    {report.url}
                                                </td>
                                        ...
                                        {errorReports.data.length === 0 && (
                                            <tr>
                                                <td
                                                    colSpan={5}
                                                    className="p-admin__table-cell--empty"
                                                >
