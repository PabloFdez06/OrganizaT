...
function calculateSubjectsPerRow(
    ...
    const raw = Math.floor(
        (containerWidthPx + gapPx) / (minCardWidthPx + gapPx),
    );
...
function pickMonthDateSelection(
    monthDate: Date,
    tasksByDate: Record<string, TaskItem[]>,
): string {
...
function flattenSubjectTasks(subject: SubjectCard | null): TaskItem[] {
    ...
    return subject.units.flatMap((unit) =>
        unit.tasks.map((task) => ({ ...task, unitName: unit.name })),
    );
}

function buildStatusClass(
    baseClass: string,
    statusKey: TaskItem['statusKey'],
    statusTone: TaskItem['statusTone'],
): string {
...
export default function Tareas({
    ...
    const [selectedSubjectId, setSelectedSubjectId] = useState<number | null>(
        initialSubjectId ?? subjectCards[0]?.id ?? null,
    );
    const [selectedDate, setSelectedDate] = useState<string>(
        calendar.selectedDate,
    );
    const [calendarMonth, setCalendarMonth] = useState<Date>(
        buildMonthDate(calendar.initialMonth),
    );
    ...
    const [visibleTaskCountBySubject, setVisibleTaskCountBySubject] = useState<
    ...
    const sideSubjects = useMemo(
        () =>
            subjectCards.filter(
                (subject) =>
                    featuredSubject === null ||
                    subject.id !== featuredSubject.id,
            ),
    ...
    useEffect(() => {
        const updateSubjectsPerRow = () => {
            ...
            const computedRootFont = window.getComputedStyle(
                document.documentElement,
            ).fontSize;
            const parsedRootFont = Number.parseFloat(computedRootFont);
            const rootFontSizePx =
                Number.isFinite(parsedRootFont) && parsedRootFont > 0
                    ? parsedRootFont
                    : FALLBACK_ROOT_FONT_SIZE_PX;
            const perRow = calculateSubjectsPerRow(
                sideGrid.clientWidth,
                rootFontSizePx,
            );
    ...
    const minimumVisibleSubjects = Math.min(
        subjectsPerRow,
        sideSubjects.length,
    );
    const visibleSideSubjects = sideSubjects.slice(0, visibleSubjectCount);
    const canLoadMoreSubjects =
        visibleSideSubjects.length < sideSubjects.length;
    const canLoadLessSubjects =
        visibleSideSubjects.length > minimumVisibleSubjects;

    const featuredTasks = useMemo(
        () => flattenSubjectTasks(featuredSubject),
        [featuredSubject],
    );

    const visibleTaskCount = featuredSubject
        ? (visibleTaskCountBySubject[featuredSubject.id] ?? TASK_BATCH_SIZE)
        : TASK_BATCH_SIZE;
    ...
    const calendarCells = useMemo(
        () => buildCalendarCells(calendarMonth, tasksByDate),
        [calendarMonth, tasksByDate],
    );

    const dayAgenda = tasksByDate[selectedDate] ?? [];
    const selectedDateObject = parseIsoDate(selectedDate);

    const selectedDateDay = selectedDateObject.toLocaleDateString('es-ES', {
        day: '2-digit',
    });
    const selectedDateWeekday = selectedDateObject
    ...
    const handleLoadMoreTasks = () => {
        ...
        setVisibleTaskCountBySubject((previous) => ({
            ...previous,
            [featuredSubject.id]:
                (previous[featuredSubject.id] ?? TASK_BATCH_SIZE) +
                TASK_BATCH_SIZE,
        }));
    };
                    ...
                    {loading && (
                        ...
                        >
                            <Alert className="p-tareas__loading-alert">
                                ...
                                <AlertDescription>
                                    Estamos procesando las tareas y el
                                    calendario desde Moodle.
                                </AlertDescription>
                            </Alert>
                    ...
                    {!loading &&
                        (featuredSubject ? (
                            <section className="p-tareas__workspace">
                                <section
                                    className="p-tareas__subjects-column"
                                    aria-label="Asignatura activa y asignaturas disponibles"
                                >
                                    <article className="p-tareas__featured">
                                        <header className="p-tareas__featured-hero">
                                            <section
                                                className="p-tareas__featured-hero-media"
                                                aria-hidden="true"
                                            >
                                                <img
                                                    className="p-tareas__featured-hero-image"
                                                    src={
                                                        FEATURED_SUBJECT_HERO_IMAGE
                                                    }
                                                    alt=""
                                                />
                                            ...
                                            >
                                                ...
                                                <rect
                                                    ...
                                                    width="100"
                                                    height="100"
                                                    fill="url(#featured-subject-gradient)"
                                                />
                                            </svg>

                                            <section className="p-tareas__featured-content">
                                                <small className="p-tareas__featured-code">
                                                    {featuredSubject.code}
                                                </small>
                                                <h2 className="p-tareas__featured-title">
                                        ...
                                        <section
                                            className="p-tareas__table"
                                            aria-label={`Tareas de ${featuredSubject.subject}`}
                                        >
                                            <header className="p-tareas__table-head">
                                                <p className="p-tareas__table-head-cell">
                                                    TAREA
                                                </p>
                                                <p className="p-tareas__table-head-cell">
                                                    FECHA ENTREGA
                                                ...
                                                <p
                                                    className="p-tareas__table-head-cell"
                                                    aria-hidden="true"
                                                />
                                            </header>
                                                            ...
                                                            (task, index) => {
                                                                const previousTask =
                                                                    index > 0
                                                                        ...
                                                                        : null;
                                                                const showUnitSeparator =
                                                                    previousTask ===
                                                                        null ||
                                                                    previousTask.unitName !==
                                                                        task.unitName;
                                                                ...
                                                                return (
                                                                    ...
                                                                    >
                                                                        {showUnitSeparator && (
                                                                            <li
                                                                                className="p-tareas__task-list-item p-tareas__task-list-item--unit-separator"
                                                                                aria-label={`Unidad ${task.unitName}`}
                                                                            >
                                                                                <span className="p-tareas__unit-separator-label">
                                                                                    {
                                                                                        task.unitName
                                                                                    }
                                                                                </span>
                                                                        ...
                                                                        <li className="p-tareas__task-list-item">
                                                                            <article className="p-tareas__task-row">
                                                                                <section className="p-tareas__task-main">
                                                                                    <h3 className="p-tareas__task-title">
                                                                                        {
                                                                                            task.name
                                                                                        }
                                                                                    </h3>
                                                                                    <p className="p-tareas__task-module">
                                                                                        Modulo:{' '}
                                                                                        {
                                                                                            task.unitName
                                                                                        }
                                                                                ...
                                                                                <p className="p-tareas__task-date">
                                                                                    {
                                                                                        task.dueLabel
                                                                                    }
                                                                                </p>
                                                                                ...
                                                                                <p
                                                                                    className={buildStatusClass(
                                                                                        'p-tareas__task-status',
                                                                                        task.statusKey,
                                                                                        task.statusTone,
                                                                                    )}
                                                                                >
                                                                                ...
                                                                                {task.url ? (
                                                                                    <a
                                                                                        className="p-tareas__task-link"
                                                                                        href={
                                                                                            task.url
                                                                                        }
                                                                                        target="_blank"
                                                                                        rel="noreferrer"
                                                                                        aria-label={`Abrir ${task.name} en Moodle`}
                                                                                    >
                                                                                        <ArrowRight
                                                                                            size={
                                                                                                16
                                                                                    ...
                                                                                    <span
                                                                                        className="p-tareas__task-link-disabled"
                                                                                        aria-hidden="true"
                                                                                    >
                                                                                        <ArrowRight
                                                                                            size={
                                        ...
                                        {(canLoadMoreTasks ||
                                            ...
                                            <footer className="p-tareas__featured-actions">
                                                {canLoadLessTasks && (
                                                    <button
                                                        type="button"
                                                        className="p-tareas__featured-action-button"
                                                        onClick={
                                                            handleLoadLessTasks
                                                        }
                                                    >
                                ...
                                <aside
                                    className="p-tareas__sidebar"
                                    aria-label="Calendario y detalle diario"
                                >
                                    <section
                                        className="p-tareas__export-block"
                                        aria-label="Exportacion de calendario academico"
                                    >
                                        {moodleConnected ? (
                                            <a
                                                className="p-tareas__export-button"
                                                href="/tareas/export-all.ics"
                                            >
                                                <Download
                                                    size={16}
                                                    aria-hidden="true"
                                            ...
                                            <button
                                                type="button"
                                                className="p-tareas__export-button p-tareas__export-button--disabled"
                                                disabled
                                            >
                                                <Download
                                                    size={16}
                                        ...
                                        <p className="p-tareas__export-note">
                                            En Google Calendar: configura e
                                            importa el archivo .ics para cargar
                                            todas tus tareas.
                                        </p>
                                    </section>
                                    ...
                                    <article className="p-tareas__calendar">
                                        <header className="p-tareas__calendar-header">
                                            <h3 className="p-tareas__calendar-title">
                                                {formatMonthLabel(
                                                    calendarMonth,
                                                )}
                                            </h3>
                                            <section className="p-tareas__calendar-nav">
                                                <button
                                                    type="button"
                                                    className="p-tareas__calendar-nav-button"
                                                    onClick={() =>
                                                        handleChangeMonth(-1)
                                                    }
                                        ...
                                        <section
                                            className="p-tareas__calendar-grid"
                                            role="grid"
                                        ...
                                        >
                                            {[
                                                ...
                                                <p
                                                    key={label}
                                                    className="p-tareas__calendar-weekday"
                                                    role="columnheader"
                                                >
                                                    {label}
                                                </p>
                                            ...
                                            {calendarCells.map((cell) => {
                                                ...
                                                const className = [
                                                    ...
                                                    cell.isCurrentMonth
                                                        ? ''
                                                        : 'p-tareas__calendar-day--outside',
                                                    isSelected
                                                        ? 'p-tareas__calendar-day--selected'
                                                        : '',
                                                ]
                                                    .filter(Boolean)
                                                    .join(' ');
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        <span className="p-tareas__calendar-day-label">
                                                            {`${cell.day}`.padStart(
                                                                2,
                                                                '0',
                                                            )}
                                                        ...
                                                        {cell.markerTone && (
                                                            <i
                                                                className={`p-tareas__calendar-day-marker p-tareas__calendar-day-marker--${cell.markerTone}`}
                                                                aria-hidden="true"
                                                            />
                                                        )}
                                                    </button>
                                    ...
                                    <article className="p-tareas__day-detail">
                                        <header className="p-tareas__day-detail-header">
                                            <strong className="p-tareas__day-detail-day">
                                                {selectedDateDay}
                                            </strong>
                                            <section className="p-tareas__day-detail-meta">
                                                <h4 className="p-tareas__day-detail-weekday">
                                                    {selectedDateWeekday}
                                                </h4>
                                                <p className="p-tareas__day-detail-label">
                                                    DETALLES DEL DIA
                                                </p>
                                            </section>
                                        </header>
                                            ...
                                            {dayAgenda.length > 0 ? (
                                                <ul className="p-tareas__day-list-items">
                                                    {dayAgenda.map((task) => (
                                                        <li
                                                            key={`agenda-${task.id}`}
                                                            className={`p-tareas__day-list-item p-tareas__day-list-item--${task.statusTone}`}
                                                        >
                                                            <small className="p-tareas__day-list-time">
                                                                {task.dueLabel}
                                                            </small>
                                                            <h5 className="p-tareas__day-list-task">
                                                                {task.name}
                                                            </h5>
                                                            <p className="p-tareas__day-list-meta-text">
                                                                {
                                                                    task.courseName
                                                                }{' '}
                                                                ·{' '}
                                                            ...
                                                            {task.url && (
                                                                <a
                                                                    className="p-tareas__day-list-link"
                                                                    href={
                                                                        task.url
                                                                    }
                                                                    target="_blank"
                                ...
                                {sideSubjects.length > 0 && (
                                    <section
                                        className="p-tareas__side-grid"
                                        aria-label="Asignaturas contraidas"
                                        ref={sideGridRef}
                                    >
                                        {visibleSideSubjects.map((subject) => (
                                            <article
                                                key={subject.id}
                                                className="p-tareas__side-card"
                                            >
                                                <button
                                                    type="button"
                                                    className="p-tareas__side-card-trigger"
                                                    onClick={() =>
                                                        handleSelectSubject(
                                                            subject.id,
                                                        )
                                                ...
                                                >
                                                    <figure
                                                        className="p-tareas__side-media"
                                                        style={buildSubjectMediaStyle(
                                                            subject,
                                                        )}
                                                        aria-hidden="true"
                                                    />
                                                    <section className="p-tareas__side-body">
                                                        <small className="p-tareas__side-code">
                                                            {subject.code}
                                                        </small>
                                                        <h3 className="p-tareas__side-title">
                                                            {subject.subject}
                                                        </h3>
                                                        <footer className="p-tareas__side-footer">
                                                            <p className="p-tareas__side-metrics">
                                                                <strong className="p-tareas__side-count">
                                                                    {subject.totalTasks
                                                                        .toString()
                                                                        .padStart(
                                                                            2,
                                                                ...
                                                                <span className="p-tareas__side-metrics-label">
                                                                    TAREAS
                                                                </span>
                                                            </p>
                                                            <span
                                                                className="p-tareas__side-plus"
                                                                aria-hidden="true"
                                                            >
                                                                +
                                                            </span>
                            ...
                            <article className="p-tareas__empty">
                                <h3 className="p-tareas__empty-title">
