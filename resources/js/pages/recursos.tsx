...
export default function Recursos({
    ...
    const [selectedBuckets, setSelectedBuckets] = useState<MetadataBucket[]>([
        'links',
        'images',
        'files',
        'folders',
    ...
    const filteredSelectedSubject = useMemo(() => {
        ...
        const units = selectedSubject.units
            .map((unit) => ({
                ...
                resources: unit.resources.filter((resource) =>
                    selectedBuckets.includes(resource.bucket),
                ),
            }))
        ...
        return {
            ...selectedSubject,
            units,
            filteredResourceCount: units.reduce(
    ...
    const filteredTotals = useMemo(() => {
        const resources =
            filteredSelectedSubject?.units.flatMap((unit) => unit.resources) ??
            [];
        ...
        return {
            ...
            documents: resources.filter(
                (resource) =>
                    resource.kind === 'document' || resource.kind === 'archive',
            ).length,
            multimedia: resources.filter(
                (resource) => resource.kind === 'multimedia',
            ).length,
            links: resources.filter(
                (resource) => resource.kind === 'external_link',
            ).length,
        };
    }, [filteredSelectedSubject]);

    const formatDistribution = {
        documents: formatPercent(
            filteredTotals.documents,
            filteredTotals.total,
        ),
        multimedia: formatPercent(
            filteredTotals.multimedia,
            filteredTotals.total,
        ),
    ...
    const handleSelectSubject = (subjectId: number) => {
        router.visit(recursosIndex({ query: { subject_id: subjectId } }).url, {
            preserveState: false,
            preserveScroll: true,
        });
    };
    ...
    const metadataCounts = useMemo(() => {
        ...
        return {
            links: resources.filter((resource) => resource.bucket === 'links')
                .length,
            images: resources.filter((resource) => resource.bucket === 'images')
                .length,
            files: resources.filter((resource) => resource.bucket === 'files')
                .length,
            folders: resources.filter(
                (resource) => resource.bucket === 'folders',
            ).length,
            others: resources.filter((resource) => resource.bucket === 'others')
                .length,
        };
    }, [filteredSelectedSubject]);
    ...
    const downloadResourceLabel = (resource: ResourceItem): string =>
        `Descargar ${resource.name}`;
            ...
            <article className="p-recursos">
                ...
                <main className="p-recursos__container p-recursos__main">
                    <header className="p-recursos__head">
                        ...
                        <section
                            className="p-recursos__head-tools"
                            aria-label="Herramientas de recursos"
                        >
                            <section
                                className="p-recursos__total"
                                aria-label="Total de recursos disponibles"
                            >
                                <strong className="p-recursos__total-value">
                                    {summary.totalResources}
                                </strong>
                                <span className="p-recursos__total-label">
                                    Recursos totales
                                </span>
                            </section>
                        </section>
                    ...
                    {loading && (
                        ...
                        >
                            <Alert className="p-recursos__loading-alert">
                                ...
                                <AlertDescription>
                                    Estamos obteniendo materiales de Moodle para
                    ...
                    <section
                        className="p-recursos__workspace"
                        aria-label="Panel de recursos"
                    >
                        <aside
                            className="p-recursos__left-rail"
                            aria-label="Listado de asignaturas"
                        >
                            <header className="p-recursos__rail-head">
                                <h2 className="p-recursos__rail-title">
                                    ASIGNATURAS
                                </h2>

                                <section
                                    className="p-recursos__dropdowns"
                                    aria-label="Paneles de analisis y filtros"
                                >
                                    <details className="p-recursos__dropdown p-recursos__dropdown--icon">
                                        <summary
                                            className="p-recursos__dropdown-summary p-recursos__dropdown-summary--icon"
                                            aria-label="Analisis de formato"
                                            title="Analisis de formato"
                                        >
                                            <BarChart3
                                                className="p-recursos__dropdown-summary-icon"
                                                size={15}
                                                aria-hidden="true"
                                            />
                                        </summary>
                                        ...
                                        <article
                                            className="p-recursos__analysis"
                                            aria-label="Analisis de formato"
                                        >
                                            <section className="p-recursos__analysis-item">
                                                <header className="p-recursos__analysis-item-head">
                                                    <span className="p-recursos__analysis-item-label">
                                                        Documentos
                                                    </span>
                                                    <strong className="p-recursos__analysis-item-value">
                                                        {
                                                            formatDistribution.documents
                                                        }
                                                        %
                                                    </strong>
                                                </header>
                                                <progress
                                                    className="p-recursos__analysis-item-progress"
                                                    max={100}
                                                    value={
                                                        formatDistribution.documents
                                                    }
                                                    aria-label="Porcentaje de documentos"
                                                />
                                            </section>
                                            ...
                                            <section className="p-recursos__analysis-item">
                                                ...
                                                <progress
                                                    ...
                                                    aria-label="Porcentaje de multimedia"
                                                />
                                            </section>
                                            ...
                                            <section className="p-recursos__analysis-item">
                                                ...
                                                <progress
                                                    ...
                                                    aria-label="Porcentaje de enlaces"
                                                />
                                            </section>
                                        </article>
                                    </details>
                                    ...
                                    <details className="p-recursos__dropdown p-recursos__dropdown--icon">
                                        <summary
                                            className="p-recursos__dropdown-summary p-recursos__dropdown-summary--icon"
                                            aria-label="Filtro por metadatos"
                                            title="Filtro por metadatos"
                                        >
                                            <Filter
                                                className="p-recursos__dropdown-summary-icon"
                                                size={15}
                                        ...
                                        <article
                                            className="p-recursos__filters"
                                            aria-label="Filtro por metadatos"
                                        >
                                            <ul className="p-recursos__filter-list">
                                                {METADATA_FILTERS.map(
                                                    (item) => {
                                                        const isActive =
                                                            selectedBuckets.includes(
                                                                item.bucket,
                                                            );
                                                        const count =
                                                            metadataCounts[
                                                        ...
                                                        return (
                                                            <li
                                                                className="p-recursos__filter-item"
                                                                key={
                                                                    item.bucket
                                                                }
                                                            >
                                                                <button
                                                                    ...
                                                                    className={[
                                                                        ...
                                                                        isActive
                                                                            ? 'p-recursos__filter-button--active'
                                                                            : '',
                                                                    ]
                                                                        .filter(
                                                                            Boolean,
                                                                ...
                                                                >
                                                                    {item.label}
                                                                    <span className="p-recursos__filter-button-count">
                                                                        {count}
                                                                    </span>
                                                                </button>
                                                            </li>
                                    ...
                                    return (
                                        ...
                                        >
                                            <section className="p-recursos__subject-item-main">
                                                <small className="p-recursos__subject-item-code">
                                                    {getModuleLabel(
                                                        subject.code,
                                                        index,
                                                    )}
                                                </small>
                                                <h3 className="p-recursos__subject-item-title">
                                                    {subject.subject}
                                                </h3>
                                            </section>
                                        </button>
                        ...
                        <section
                            className="p-recursos__content"
                            aria-label="Recursos por asignatura"
                        >
                                ...
                                <article className="p-recursos__subject p-recursos__subject--featured">
                                    <header className="p-recursos__subject-head">
                                        <section className="p-recursos__subject-head-main">
                                            <small className="p-recursos__subject-head-code">
                                                {filteredSelectedSubject.code}
                                            </small>
                                            <h2 className="p-recursos__subject-head-title">
                                                {
                                                    filteredSelectedSubject.subject
                                                }
                                            </h2>
                                        </section>
                                    </header>
                                    ...
                                    <section className="p-recursos__subject-body">
                                        {filteredSelectedSubject.units.length >
                                        0 ? (
                                            filteredSelectedSubject.units.map(
                                                (unit, unitIndex) => (
                                                    <section
                                                        key={`${filteredSelectedSubject.id}-${unit.name}`}
                                                        className="p-recursos__unit"
                                                    >
                                                        <header className="p-recursos__unit-head">
                                                            <span className="p-recursos__unit-index">{`${`${unitIndex + 1}`.padStart(2, '0')}`}</span>
                                                            <h3 className="p-recursos__unit-title">
                                                                {unit.name}
                                                            </h3>
                                                        </header>
                                                                        ...
                                                                        'folder' ? (
                                                                            <article className="p-recursos__folder-card">
                                                                                ...
                                                                                >
                                                                                    <section className="p-recursos__folder-main">
                                                                                        <section
                                                                                            className={`p-recursos__resource-icon ${kindToneClass(resource.kind)}`}
                                                                                            aria-hidden="true"
                                                                                        >
                                                                                            {kindIcon(
                                                                                                resource.kind,
                                                                                        ...
                                                                                        <section className="p-recursos__resource-content">
                                                                                            <p className="p-recursos__resource-name">
                                                                                                {
                                                                                                    resource.name
                                                                                                }
                                                                                            </p>
                                                                                                ...
                                                                                                (
                                                                                                    ...
                                                                                                    >
                                                                                                        <article className="p-recursos__resource-card p-recursos__resource-card--child">
                                                                                                            <section
                                                                                                                className={`p-recursos__resource-icon ${kindToneClass(child.kind)}`}
                                                                                                                aria-hidden="true"
                                                                                                            >
                                                                                                                {kindIcon(
                                                                                                                    child.kind,
                                                                                                            ...
                                                                                                            <section className="p-recursos__resource-content">
                                                                                                                ...
                                                                                                                <small className="p-recursos__resource-meta">
                                                                                                                    {child.sizeLabel
                                                                                                                        ? `${child.sizeLabel} / ${child.kindLabel}`
                                                                                                                        : child.kindLabel}
                                                                                                                </small>
                                                                                                            </section>
                                                                                                            ...
                                                                                                            {child.downloadUrl ? (
                                                                                                                <a
                                                                                                                    ...
                                                                                                                    aria-label={downloadResourceLabel(
                                                                                                                        child,
                                                                                                                    )}
                                                                                                                    download
                                                                                                                >
                                                                                                                ...
                                                                                                                <span
                                                                                                                    className="p-recursos__resource-disabled"
                                                                                                                    aria-hidden="true"
                                                                                                                >
                                                                                                                    <Folder
                                                                                                                        size={
                                                                            ...
                                                                            <article className="p-recursos__resource-card">
                                                                                ...
                                                                                <section className="p-recursos__resource-content">
                                                                                    ...
                                                                                    <small className="p-recursos__resource-meta">
                                                                                        {resource.sizeLabel
                                                                                            ? `${resource.sizeLabel} / ${resource.kindLabel}`
                                                                                            : resource.kindLabel}
                                                                                    </small>
                                                                                </section>
                                            ...
                                            <article className="p-recursos__empty p-recursos__empty--inline">
                                                <h3 className="p-recursos__empty-title">
                                                    No hay recursos para esta
                                                    seleccion
                                                </h3>
                                                <p className="p-recursos__empty-description">
                                                    Puede deberse a que los
                                                    filtros estan ocultando el
                                                    contenido o a que esta
                                                    asignatura aun no tiene
                                                    material publicado.
                                                </p>
                                            </article>
                                        )}
                            ...
                            {!loading && subjects.length === 0 && (
                                <article className="p-recursos__empty">
                                    ...
                                    <ul className="p-recursos__empty-reasons">
                                        <li>
                                            Moodle no esta conectado o la sesion
                                            ha caducado.
                                        </li>
                                        <li>
                                            La asignatura todavia no tiene
                                            archivos, enlaces o carpetas
                                            compartidas.
                                        </li>
                                        <li>
                                            Los filtros de metadatos estan
                                            ocultando todos los tipos de recurso
                                            disponibles.
                                        </li>
                                        <li>
                                            Puede haber restricciones de
                                            permisos o una sincronizacion
                                            pendiente en Moodle.
                                        </li>
                                    </ul>
                                </article>
