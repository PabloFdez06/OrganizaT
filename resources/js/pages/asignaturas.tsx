...
function buildColumnSpans(total: number): ColumnSpan[] {
    ...
    if (remainder !== 0) {
        ...
        spans[lastIndex] = Math.min(
            3,
            spans[lastIndex] + missingColumns,
        ) as ColumnSpan;
    }
        ...
        <>
            ...
            <article className="p-asignaturas">
                ...
                <main className="p-asignaturas__container p-asignaturas__main">
                    <header className="p-asignaturas__hero-head">
                        <section>
                            <h1 className="p-asignaturas__hero-title">
                                ASIGNATURAS
                                <span className="p-asignaturas__hero-title-dot">
                                    .
                                </span>
                            </h1>
                        </section>
                    ...
                    {loading && (
                        <section
                            className="p-asignaturas__loading"
                            aria-live="polite"
                            aria-busy="true"
                        >
                            <Alert className="p-asignaturas__loading-alert">
                                ...
                                <AlertDescription>
                                    Estamos cargando tus datos de Moodle en
                                    segundo plano.
                                </AlertDescription>
                            </Alert>
                            ...
                            <section className="p-asignaturas__loading-grid">
                                {Array.from({ length: 3 }).map((_, index) => (
                                    <article
                                        key={`asig-skeleton-${index}`}
                                        className="p-asignaturas__loading-card p-asignaturas__course p-asignaturas__course--base"
                                    >
                    ...
                    <StatsStrip
                        ...
                        items={[
                            ...
                            {
                                label: 'ASIGNATURAS >= 75%',
                                value: summary.highProgress,
                                highlight: true,
                            },
                            {
                    ...
                    <section className="p-asignaturas__subjects">
                        <section className="p-asignaturas__grid">
                            {courseCards.map((course, index) => {
                                const variant =
                                    variantSequence[index] ?? 'base';
                                const span = columnSpans[index] ?? 1;
                                const courseImageUrl = toMoodleMediaUrl(
                                    course.image,
                                );
                                ...
                                return (
                                    ...
                                    >
                                        {cardVariantClass === 'hero' && (
                                            <span className="p-asignaturas__badge">
                                                En curso
                                            </span>
                                        )}
                                        {cardVariantClass === 'image' &&
                                            courseImageUrl && (
                                                <figure className="p-asignaturas__image">
                                                    <img
                                                        className="p-asignaturas__course-image"
                                                        src={courseImageUrl}
                                        ...
                                        <section className="p-asignaturas__course-content">
                                            <h3 className="p-asignaturas__course-title">
                                                {course.title}
                                            </h3>

                                            <section className="p-asignaturas__meta">
                                                <span className="p-asignaturas__teacher-label">
                                                    Docente
                                                </span>
                                                <span className="p-asignaturas__teacher">
                                                    {course.teacher}
                                                </span>
                                            </section>

                                            <section
                                                className="p-asignaturas__progress"
                                                aria-label={`Progreso de ${course.title}`}
                                            >
                                                <header className="p-asignaturas__progress-head">
                                                    <span className="p-asignaturas__progress-label">
                                                        Progreso
                                                    </span>
                                                    <strong className="p-asignaturas__progress-value">
                                                        {course.progress}%
                                                    </strong>
                                                </header>
                                                <span
                                                    className="p-asignaturas__progress-track"
                                                    aria-hidden="true"
                                                >
                                                    <i
                                                        className="p-asignaturas__progress-fill"
                                                        style={{
                            ...
                            {!loading && courseCards.length === 0 && (
                                <article className="p-asignaturas__empty">
                                    <h3 className="p-asignaturas__empty-title">
