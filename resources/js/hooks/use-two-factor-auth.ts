...
export const useTwoFactorAuth = ({
    ...
    const redirectToPasswordConfirmation = useCallback(
    ...
    const refreshSetupData = useCallback(async (): Promise<void> => {
        ...
        try {
            const [qrResponse, secretResponse, codesResponse] =
                await Promise.all([
                    requestJson<TwoFactorSetupData | Record<string, never>>(
                        qrCode.url(),
                    ),
                    requestJson<TwoFactorSecretKey | Record<string, never>>(
                        secretKey.url(),
                    ),
                    requestJson<string[] | Record<string, never>>(
                        recoveryCodes.url(),
                    ),
                ]);

            setQrCodeSvg(
                'svg' in qrResponse && typeof qrResponse.svg === 'string'
                    ? qrResponse.svg
                    : null,
            );
            setManualSetupKey(
                'secretKey' in secretResponse &&
                    typeof secretResponse.secretKey === 'string'
                    ? secretResponse.secretKey
                    : null,
            );
            setRecoveryCodesList(
                Array.isArray(codesResponse) ? codesResponse : [],
            );
        } catch (error) {
    ...
    const disableTwoFactor = useCallback(async (): Promise<boolean> => {
        ...
        try {
            ...
            setErrors({
                general: 'No se pudo desactivar el 2FA. Inténtalo de nuevo.',
            });
