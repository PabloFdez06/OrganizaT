...
export const rules = {
    required:
        (msg = 'Este campo es obligatorio.'): ValidationRule =>
        (v) =>
            v.trim().length > 0 ? null : msg,

    email:
        (msg = 'Introduce un correo electrónico válido.'): ValidationRule =>
        (v) =>
            /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(v.trim()) ? null : msg,
    ...
    matches:
        (field: string, msg = 'Los valores no coinciden.'): ValidationRule =>
        (v, all) =>
