...
export function formatFeedbackToBlocks(feedback: string): FeedbackBlock[] {
    ...
    while (lineIndex < lines.length) {
        ...
        if (isOrderedListStart || isUnorderedListStart) {
            ...
            const listPattern = isOrderedListStart
                ? /^\d+[.)]\s+/
                : /^[-*•]\s+/;

            while (
                lineIndex < lines.length &&
                listPattern.test(lines[lineIndex].trim())
            ) {
                listItems.push(
                    lines[lineIndex].trim().replace(listPattern, '').trim(),
                );
                lineIndex += 1;
            }
        ...
        while (lineIndex < lines.length) {
            ...
            if (
                ...
                isSpecialBlockStart(paragraphCandidate)
            ) {
                break;
            }
