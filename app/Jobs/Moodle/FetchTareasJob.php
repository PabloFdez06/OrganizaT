...
class FetchTareasJob implements ShouldQueue
{
    use Dispatchable;
    ...
    public function handle(
        ...
        if (! $sessionService->hasActiveSession($user)) {
            $asyncCache->markError('tareas', $user->id, 'Tu sesión de Moodle no está activa. Vuelve a conectar tu cuenta.');
        ...
        try {
            ...
            $asyncCache->markDone('tareas', $user->id, [
                'academicPayload' => $payload,
            ]);
        } catch (MoodleAuthenticationException|MoodleRequestException $exception) {
            $asyncCache->markError('tareas', $user->id, $exception->getMessage());
        } catch (\Throwable $exception) {
            Log::error('Error al obtener tareas en segundo plano.', [
                'user_id' => $user->id,
                'message' => $exception->getMessage(),
            ]);

            $asyncCache->markError('tareas', $user->id, 'No se pudieron cargar las tareas en este momento.');
        }
    }
}
