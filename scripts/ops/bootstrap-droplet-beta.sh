...
validate_env() {
    local required_vars
    required_vars="APP_URL DB_DATABASE DB_USERNAME DB_PASSWORD DB_ROOT_PASSWORD REDIS_PASSWORD APP_IMAGE NGINX_IMAGE"
    ...
    if [ -z "$(read_env_value "IMAGE_TAG")" ]; then
        set_env_value "IMAGE_TAG" "latest"
        log "IMAGE_TAG no definido. Se usara latest."
    fi
}
...
deploy_stack() {
    ...
    log "Descargando imagenes publicadas..."
