...
6. APP_IMAGE, NGINX_IMAGE e IMAGE_TAG para reutilizar imágenes publicadas en GHCR.
...
1. deploy.yml: valida calidad backend, ejecuta checks y tests frontend, corre tests backend, construye y publica imágenes en GHCR y despliega por SSH.

### Flujo de deploy

1. Verifica estructura del proyecto en la raíz del repositorio.
2. Ejecuta `composer lint:check`.
3. Ejecuta `npm run cicheck`.
4. Prepara SQLite local y lanza `php artisan test`.
5. Construye imágenes runtime y nginx.
6. Publica imágenes en ghcr.io.
7. Conecta por SSH al servidor de producción.
8. Actualiza la rama por defecto del repositorio.
9. Hace pull de imágenes ya construidas y levanta servicios.
10. Ejecuta `migrate --force`, `optimize` y `queue:restart`.

## 8.6 Proceso de despliegue documentado

> Los scripts operativos ya no recompilan en remoto: consumen las imágenes publicadas en GHCR y solo hacen `pull` + `up`.
...
4. Imágenes de despliegue ausentes: bootstrap y deploy fallan pronto si faltan APP_IMAGE o NGINX_IMAGE.
5. Drift de estado: inspect-beta.sh centraliza revision de logs.
